...
var Vec4 = __webpack_require__(0).Vec4;
...
var Core = function(options) {
    var CanvasGL = __webpack_require__(26);
    ...
    this.MLib = {
        Vec3: __webpack_require__(0).Vec3,
        Vec4: __webpack_require__(0).Vec4,
        Mat4: __webpack_require__(3),
...
var Poly = function() {
    ...
    this.drawType = 'TRIANGLE_STRIPS';
    this.geometry = [];

    this.getModel = function() {
        var tmp = [];

        this.geometry.forEach(function(renderable) {
            tmp.push(
                renderable.vertex.x,
                renderable.vertex.y,
                renderable.vertex.z,
    ...
    this.setGeometry = function(m) {
        var color = Vec4.New(0.8, 0.8,  0.8, 1.0);

        this.geometry.push(new Point(m.row1, color, {
            u: 0,
            v: 1
        }));

        this.geometry.push(new Point(m.row2, color, {
            u: 1,
            v: 1
        }));

        this.geometry.push(new Point(m.row3, color, {
            u: 1,
            v: 0
        }));
    ...
    this.cube = function(size, dx) {
        ...
        var m = makeModule(m1, this.roty, this.rotz, dx);


        cube.add(m);

        cube.add( mirrorModule(m, tfront,  this.rotz, dx) );

        cube.add( mirrorModule(m, tleft,  this.roty, dx) );
        cube.add( mirrorModule(m, tright, this.roty, dx) );

        cube.add( mirrorModule(m, tup, this.rotx, dx) );
        cube.add( mirrorModule(m, tdown, this.rotx, dx) );
...
(function(factory) {
  ...
  if (true) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(6), __webpack_require__(38), exports], __WEBPACK_AMD_DEFINE_RESULT__ = function(_, $, exports) {
...
var DemoRouter = Backbone.Router.extend({
    ...
    noname: function(){
        this.clear();
        var demo = __webpack_require__(14);
        demo.init();
